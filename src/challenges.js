...
wordsToSearch = "trouble";

function howManyTimes(repeatedWords, wordsToSearch) {
  ...
  for (let i = 0; i < repeatedWords.length; i++) {
    if (repeatedWords[i] === wordsToSearch) {
      count = count + 1;
    }
  }
  return count;
...
function multiplyBy(numbers, multi) {
  ...
  numbers.forEach(function (changedNumber) {
    newArray.push(changedNumber * multi);
  });
  return newArray;
}
